...
function openModal(index) {
  ...
  // ⭐ 重置遮罩與圖片狀態
  const cover = modal.querySelector(".question-cover");
  const img = modal.querySelector(".question-image");
...
document.querySelectorAll(".success").forEach(btn => {
  btn.addEventListener("click", e => {
    ...
    if (data.type === "chance" || data.type === "fate") {
      bonusPoints[pendingChallenger]++;
      updateBonusDisplay(pendingChallenger);
    }
...
document.querySelectorAll(".modal-content").forEach(modal => {
  ...
  modal.addEventListener("mousedown", e => {
    ...
    modal.style.transform = "none"; // ⭐ 關掉置中 transform
...
document.addEventListener("mousemove", e => {
  ...
  document.querySelectorAll(".land-modal.show .modal-content")
    .forEach(modal => {
...
document.querySelectorAll(".show-question").forEach(btn => {
  btn.addEventListener("click", e => {
    ...
    const cover = modalContent.querySelector(".question-cover");
    const img = modalContent.querySelector(".question-image");
...
function updateBonusDisplay(player) {
  const bonusEl = document.getElementById(`bonus-${player}`);
  const totalEl = document.getElementById(`score-${player}-total`);
  if (!bonusEl || !totalEl) return;

  if (bonusPoints[player] > 0) {
    bonusEl.textContent = `+${bonusPoints[player]}`;
  } else {
    bonusEl.textContent = "";
  }
